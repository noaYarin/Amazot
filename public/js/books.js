$(function () {
  $.get("/authors", (authors) => {
    $.each(authors, (i, value) => {
      $("#selectList").append(
        $("<option></option>").text(value.name).attr("value", value._id)
      );
    });
  });
  ...
  $('[ data-role="FindAllBtn"]').click(() => {
    $.get("/books", (allBooks) => {
      ...
      $.each(allBooks, (_, field) => {
        let value = JSON.stringify(field);
        let parseValue = JSON.parse(value);
        table.append(
          `<tr><td>${parseValue._id}</td><td>${parseValue.name}</td><td>${parseValue.publish_date}</td><td>${parseValue.author.name}</td><td>${parseValue.isInStock}</td></tr>`
        );
      });
  ...
  let displayBooksData = (books) => {
    let date = new Date(books.publish_date).toISOString().slice(0, 10);
    $('[data-role="name"]').val(books.name),
      $("#selectList").val(books.author),
      $('input[type="checkbox"]').prop("checked", books.isInStock),
